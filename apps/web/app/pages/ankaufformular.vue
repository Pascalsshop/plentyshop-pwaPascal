<template>
  <AmikonInformationAppearance>
    <h1 class="text-3xl font-black tracking-tight text-slate-950">
      {{ english ? 'Offer equipment for purchase' : 'Ankaufformular' }}
    </h1>
    ...
    <div class="mt-8 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      <h2 class="text-lg font-bold text-slate-950">{{ english ? 'Purchase inquiries' : 'Kontakt für Ankauf' }}</h2>
      <ul class="mt-3 space-y-2 text-slate-700">
        <li>
          <span class="font-semibold">{{ english ? 'Email:' : 'E-Mail:' }}</span>
          <a href="mailto:info@amikon.de">info@amikon.de</a>
        </li>
        <li>{{ english ? 'Business customers only.' : 'Nur für Gewerbekunden.' }}</li>
      </ul>
